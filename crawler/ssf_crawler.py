...
class SSFCrawler:
    ...
    def add_items_to_restock_check_list(self, brand):
        ...
        for item in self.items:
            restock_check_list.append([item.name, item.brand, item.img_url, item.url, item.id, False])
    ...
    def get_restock_check_items(self, brand):
        ...
        for item in restock_check_list:
            if item[5] == True:
                restock_item = SSFItem(name=item[0], brand=item[1], price="", discount="", img_url=item[2], url=item[3], id=item[4], options=[])
